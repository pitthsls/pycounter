...
class TSVJR1(unittest.TestCase):
    ...
    def test_stats(self):
        ...
        self.assertEqual([x[2] for x in publication], [1, 4, 9, 4, 2, 0, 1, 4, 5, 2])
        publication = self.report.pubs[1]
        self.assertEqual([x[2] for x in publication], [3, 0, 0, 0, 1, 0, 14, 1, 3, 5])
    ...
    def test_period(self):
        self.assertEqual(
            self.report.period, (datetime.date(2013, 1, 1), datetime.date(2013, 10, 31))
        )

    def test_dates(self):
        publication = self.report.pubs[0]
        self.assertEqual(
            [x[0] for x in publication],
            [
